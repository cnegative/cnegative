...
typedef struct cn_allocator {
    cn_mem_record *head;
    cn_mem_record *freed_head;
    cn_mem_record *freed_tail;
    cn_mem_record **buckets;
    cn_mem_record **freed_buckets;
    size_t bucket_capacity;
    size_t freed_bucket_capacity;
    size_t live_blocks;
    size_t live_bytes;
    size_t peak_bytes;
    size_t quarantine_blocks;
...
#define CN_REALLOC(allocator, ptr, type, count) \
    ((type *)cn_realloc_mul_impl((allocator), (ptr), sizeof(type), (count), __FILE__, __LINE__))
