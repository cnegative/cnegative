...
- `int`, `u8`, `bool`, `str`, arrays, slices, structs, `ptr`, `result`, and source-level `null` pointer literals.
- array size expressions resolved from integer constants before typed IR lowering, plus `[value; N]` repeat array literals.
...
- `print(...)`, `println(...)`, `input()`, `str_copy(...)`, `str_concat(...)`, and string equality lowered through embedded LLVM runtime helpers backed by libc.
...
- `std.strings.copy(...)` and `std.strings.concat(...)` lower to the same owned-string helpers as the global forms. `std.strings.from_int(...)` lowers through a small integer-formatting helper that returns an owned runtime string.
- `std.text` lowers to builder helpers layered on top of the byte-buffer runtime. `std.text.append(...)` copies string bytes into the builder, `std.text.append_int(...)` formats and appends an integer without forcing a temporary `str` in user code, `std.text.push_byte(...)` appends one byte, `std.text.view(...)` returns a non-owning slice, and `std.text.build(...)` returns an owned runtime string on success.
...
- builtin `print(...)` now lowers to the same no-newline write family, while builtin `println(...)` lowers to the newline-print family.
...
- `main` entry wrapping currently supports `void`, `int`, `u8`, `result int`, and `result u8`. `err` from `main` maps to process exit code `1`.
